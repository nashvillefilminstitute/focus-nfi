...
function applyFilmFilter(file, maxWidth=900, quality=0.60) {
  return new Promise((resolve) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        // 1. Get pixel data and apply warm tone + saturation boost
        const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
        const data = imageData.data;
        for (let i = 0; i < data.length; i += 4) {
          let r = data[i], g = data[i+1], b = data[i+2];
          // Warm shift — boost reds/yellows, pull back blues
          r = Math.min(255, r * 1.12 + 15);
          g = Math.min(255, g * 1.05 + 5);
          b = Math.max(0, b * 0.82 - 10);
          // Saturation boost
          const avg = (r + g + b) / 3;
          r = Math.min(255, avg + (r - avg) * 1.25);
          g = Math.min(255, avg + (g - avg) * 1.25);
          b = Math.min(255, avg + (b - avg) * 1.25);
          // Slight vignette via brightness reduction at edges handled below
