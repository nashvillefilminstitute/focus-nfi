...
function NFILogo({ size = 44 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 512 512" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect x="5" y="5" width="502" height="502" stroke="#c9a84c" strokeWidth="5" fill="none"/>
      <rect x="13" y="13" width="486" height="486" stroke="#785e23" strokeWidth="2" fill="none"/>
      <rect x="30" y="120" width="452" height="272" rx="4" fill="#f5f0e8"/>
      <rect x="30" y="120" width="48" height="272" fill="#d2c6ae"/>
      <rect x="434" y="120" width="48" height="272" fill="#d2c6ae"/>
      <rect x="30" y="120" width="452" height="34" fill="#e1d6be"/>
      <rect x="30" y="120" width="452" height="4" fill="#c9a84c"/>
      <rect x="282" y="68" width="132" height="58" rx="3" fill="#e1d6be"/>
      <rect x="282" y="68" width="132" height="58" rx="3" stroke="#c9a84c" strokeWidth="3" fill="none"/>
      <rect x="292" y="78" width="112" height="38" rx="2" fill="#111"/>
      <rect x="35" y="140" width="40" height="30" fill="#000" stroke="#785e23" strokeWidth="1"/>
      <rect x="35" y="332" width="40" height="30" fill="#000" stroke="#785e23" strokeWidth="1"/>
      <rect x="438" y="168" width="46" height="38" rx="2" fill="#e1d6be" stroke="#785e23" strokeWidth="1.5"/>
      <rect x="438" y="288" width="46" height="38" rx="2" fill="#e1d6be" stroke="#785e23" strokeWidth="1.5"/>
      <circle cx="192" cy="256" r="92" fill="#c9a84c"/>
      <circle cx="192" cy="256" r="84" fill="#1a1a1a"/>
      <circle cx="192" cy="256" r="72" fill="#e1d6be"/>
      <circle cx="192" cy="256" r="60" fill="#333"/>
      <circle cx="192" cy="256" r="46" fill="#0e1218"/>
      <circle cx="192" cy="256" r="28" fill="#000"/>
      <circle cx="212" cy="234" r="13" fill="#c8dcff" fillOpacity="0.6"/>
      <rect x="88" y="348" width="336" height="7" fill="#c9a84c"/>
      <text x="410" y="344" textAnchor="end" fill="#000000"
        fontFamily="Arial,sans-serif" fontWeight="900" fontSize="68" letterSpacing="2">NFI</text>
      <text x="256" y="430" textAnchor="middle" fill="#c9a84c"
        fontFamily="Arial,sans-serif" fontWeight="900" fontSize="62" letterSpacing="18">FOCUS</text>
    </svg>
  );
}
...
function applyFilmFilter(file, maxWidth=1400, quality=0.78) {
  return new Promise((resolve) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        // 1. Pixel-level adjustments — less yellow, deeper shadows
        const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
        const data = imageData.data;
        for (let i = 0; i < data.length; i += 4) {
          let r = data[i], g = data[i+1], b = data[i+2];
          // Crush shadows — darken anything below midtones
          const lum = 0.299*r + 0.587*g + 0.114*b;
          const shadowFactor = lum < 100 ? 0.72 : lum < 160 ? 0.88 : 1.0;
          r *= shadowFactor; g *= shadowFactor; b *= shadowFactor;
          // Very subtle warm shift — much less yellow than before
          r = Math.min(255, r * 1.04 + 4);
          g = Math.min(255, g * 1.01);
          b = Math.max(0, b * 0.94 - 3);
          // Mild saturation boost
          const avg = (r + g + b) / 3;
          r = Math.min(255, avg + (r - avg) * 1.15);
          g = Math.min(255, avg + (g - avg) * 1.15);
          b = Math.min(255, avg + (b - avg) * 1.15);
        ...
        // 3. Random light leak streak (60% chance)
        if (Math.random() > 0.4) {
          const leakX = Math.random() * w;
          const leakW = w * (0.15 + Math.random() * 0.3);
          const leakColors = [
            ["rgba(255,160,60,0)","rgba(255,160,60,0.18)","rgba(255,200,80,0.08)","rgba(255,160,60,0)"],
            ["rgba(255,80,40,0)","rgba(255,80,40,0.15)","rgba(255,120,60,0.07)","rgba(255,80,40,0)"],
            ["rgba(200,140,255,0)","rgba(200,140,255,0.12)","rgba(220,160,255,0.06)","rgba(200,140,255,0)"],
          ];
          const cols = leakColors[Math.floor(Math.random()*leakColors.length)];
          const leak = ctx.createLinearGradient(leakX - leakW/2, 0, leakX + leakW/2, h * 0.6);
          cols.forEach((c, i) => leak.addColorStop(i/3, c));
          ctx.fillStyle = leak;
          ctx.fillRect(0, 0, w, h);
        }
        ...
        for (let i = 0; i < gd.length; i += 4) {
          ...
          gd[i] = Math.max(0, Math.min(255, gd[i] + grain));
          gd[i+1] = Math.max(0, Math.min(255, gd[i+1] + grain));
          gd[i+2] = Math.max(0, Math.min(255, gd[i+2] + grain));
        }
        ctx.putImageData(grainData, 0, 0);

        // 5. Dust particles (8-16 random specks)
        const dustCount = 8 + Math.floor(Math.random() * 8);
        ctx.fillStyle = "rgba(255,240,200,0.35)";
        for (let i = 0; i < dustCount; i++) {
          const dx = Math.random() * w;
          const dy = Math.random() * h;
          const dr = 0.5 + Math.random() * 2;
          ctx.beginPath();
          ctx.arc(dx, dy, dr, 0, Math.PI * 2);
          ctx.fill();
        }

        // 6. Subtle halation — soft glow on highlights
        ctx.filter = "blur(3px)";
...
function ComposePost({ currentUser, onPost, onClose }) {
  ...
  async function handleFileSelect(e) {
    ...
    if (isVideo) {
      ...
      video.onloadedmetadata = () => {
        if (video.duration > 30) { alert("Video must be 30 seconds or less."); return; }
        setMediaFile(file);
        setMediaPreview(url);
        setMediaType("video");
      };
    } else {
      // Apply cinematic film filter
  ...
  return (
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal slide-up" style={{maxWidth:520,display:"flex",flexDirection:"column",maxHeight:"92vh"}}>
        ...
        <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:16,paddingTop:14,borderTop:`1px solid ${G.border}`,flexShrink:0}}>
    ...
    <div className="stagger">
      ...
      {showAdd&&(
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setShowAdd(false)}>
          <div className="modal slide-up" style={{display:"flex",flexDirection:"column",maxHeight:"90vh"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexShrink:0}}>
              <h3 style={{fontWeight:800,fontSize:18}}>Add Event</h3>
              <button onClick={()=>setShowAdd(false)} style={{background:"transparent",color:G.muted,fontSize:18,cursor:"pointer",fontFamily:G.sans}}>✕</button>
            </div>
